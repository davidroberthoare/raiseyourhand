...
$(".roleSection.role-player").on("click", ".style-mult .answerBtn", function () {
	if (QUESTION.plural == true) {
		//allow multiple choices per question
...
// when a plural-mult answer is submitted...
$(".roleSection.role-player").on("click", ".style-mult #pluralSubmitBtn", function () {
  var chosen = [];
  $(".style-mult .answerBtn.selected").each(function(){
    chosen.push($(this).data("id"));
  })

  console.log("CHOSEN:", chosen)
  ROOM.send("submitAnswer", chosen);
  p_showResults();
});
...
function setupBlankAnswers() {
	...
	MULTS.forEach((letter) => {
		...
		str += 'data-id="' + letter + '" ';
		str +=
			'style="background-color: ' + randomColor({ luminosity: "dark", seed: letter + letter + letter }) + ';">';
  ...
  // switch (CHARTTYPE) {
		...
		// case "pie":
			if (ANSWERS && Object.keys(ANSWERS).length > 0) {
				console.log("drawing chart with these answers...", CHARTTYPE, ANSWERS);
				...
				// RESULTSCHART.data.datasets[0].data = Object.values(ANSWERS);
